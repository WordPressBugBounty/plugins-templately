...
class API {
	...
	public function get_dependencies() {
		...
		$query = '{ dependencies{ id, name, icon, is_pro, platform, slug, plugin_file, plugin_original_slug } }';
		$data  = self::$query->get( $query );
		if ( is_wp_error( $data ) ) {
	...
	private function get_items( $type = 'items' ) {
		$pagenum      = isset( $this->post_data['pagenum'] ) ? intval( $this->post_data['pagenum'] ) : 1;
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash
		$platform     = isset( $this->post_data['platform'] ) ? strtolower( sanitize_text_field($this->post_data['platform']) ) : 'elementor';
		$package_type = isset( $this->post_data['plan_type'] ) ? strtolower( sanitize_text_field($this->post_data['plan_type']) ) : false;
		$per_page     = isset( $this->post_data['per_page'] ) ? intval( $this->post_data['per_page'] ) : 48;
		$platform     = $platform === 'templately' ? 'elementor' : $platform;
		$category_id  = isset( $this->post_data['category_id'] ) ? intval( $this->post_data['category_id'] ) : false;
		$dependencies = isset( $this->post_data['dependencies'] ) ? sanitize_text_field( $this->post_data['dependencies'] ) : false;
		$tag_id       = isset( $this->post_data['tag_id'] ) ? json_encode( sanitize_text_field($this->post_data['tag_id']) ) : false;
	...
	private function getItemsOrPacks() {
		$pagenum  = isset( $this->post_data['pagenum'] ) ? intval( $this->post_data['pagenum'] ) : 1;
		$platform = isset( $this->post_data['platform'] ) ? strtolower( sanitize_text_field($this->post_data['platform']) ) : 'elementor';
		$search   = isset( $this->post_data['search'] ) ? strtolower( sanitize_text_field( $this->post_data['search'] ) ) : '';
		$platform = $platform === 'templately' ? 'elementor' : $platform;

		$category_id  = isset( $this->post_data['category_id'] ) ? intval( $this->post_data['category_id'] ) : false;
		$dependencies = isset( $this->post_data['dependencies'] ) ? json_encode( sanitize_text_field( $this->post_data['dependencies'] ) ) : false;
		$tag_id       = isset( $this->post_data['tag_id'] ) ? json_encode( intval( $this->post_data['tag_id'] ) ) : false;
		$types        = isset( $this->post_data['query_type'] ) ? sanitize_text_field( $this->post_data['query_type'] ) : 'block';

		$query    = sprintf(
			'{ getItemsAndPacks( page: %s, search: "%s", per_page: 48, platform: "%s", query_type: "%s", %s%s%s ) { data { id, name, rating, type, slug, favourite_count, thumbnail, price, author{ display_name, name, joined }, dependencies, category{ id, name } } } }',
			$pagenum,
			$search,
			esc_attr( $platform ),
			$types,
	...
	public function insert_template( $origin = 'remote' ) {
		$id = isset( $this->post_data['id'] ) ? intval( $this->post_data['id'] ) : null;
		if ( is_null( $id ) ) {
			Helper::send_error( __( 'Templates ID Not Found.', 'templately' ) );
		}
		$is_editor    = isset( $this->post_data['is_editor'] ) ? $this->post_data['is_editor'] === "true" : null;
		$default_args = array (
		...
		if ( isset( $this->post_data['file_type'] ) && ! empty( $this->post_data['file_type'] ) ) {
			$default_args['file_type'] = sanitize_text_field( $this->post_data['file_type'] );
		}
	...
	private function cloud_push( $name, $data, $item_type = 'elementor', $args = array () ) {
		...
		if ( isset( $cloud_item->myCloud_item ) && 'elementor' === $item_type ) {
	...
	private function push_to_cloud( $id = null, $item_type = 'elementor', $args = [] ) {
		if ( isset( $this->post_data['workspace_id'] ) ) {
			$args['workspace_id'] = intval( $this->post_data['workspace_id'] );
		}
		...
		if ( $item_type === 'gutenberg' ) {
			$name = isset( $this->post_data['title'] ) && ! empty( $this->post_data['title'] ) ? sanitize_text_field( $this->post_data['title'] ) : '';
			$data = isset( $this->post_data['data'] ) && ! empty( $this->post_data['data'] ) ? $this->post_data['data'] : ''; // phpcs:ignore
	...
	private function remove_from_cloud( $id = null ) {
		...
		$from     = isset( $this->post_data['remove_from'] ) && ! empty( $this->post_data['remove_from'] ) ? sanitize_text_field( $this->post_data['remove_from'] ) : 'cloud';
	...
	protected function check_dependency() {
		...
		$platform = isset( $this->post_data['platform'] ) ? sanitize_text_field($this->post_data['platform']) : 'elementor';
		...
		if( ! empty( $this->post_data['dependency'] ) ) {
			$dependencies = json_decode( wp_unslash( sanitize_text_field( $this->post_data['dependency'] ) ) );
	...
	protected function get_item() {
		$slug = isset( $this->post_data['slug'] ) ? sanitize_text_field( $this->post_data['slug'] ) : false;
		if ( ! $slug ) {
			Helper::send_error( __( 'Something went wrong.', 'templately' ) );
		}
		$type = isset( $this->post_data['type'] ) ? sanitize_text_field( $this->post_data['type'] ) : 'block';
	...
	public function import_to_library() {
		$id = isset( $this->post_data['id'] ) ? intval( $this->post_data['id'] ) : false;
		if ( ! $id ) {
			Helper::send_error( __( 'Something went wrong!', 'templately' ) );
		}
		$title = isset( $this->post_data['title'] ) ? sanitize_text_field( $this->post_data['title'] ) : '';
	...
	public function login_user() {
		...
		$global_signin = isset( $this->post_data['global_signin'] ) ? filter_var( $this->post_data['global_signin'], FILTER_VALIDATE_BOOLEAN) : false;
		if( $global_signin ) {
		...
		$withApi = isset( $this->post_data['withApi'] ) ? sanitize_text_field( $this->post_data['withApi'] ) === 'true' : false;

		if ( $withApi ) {
			$api_key = '';
			if ( empty( $this->post_data['api_key'] ) ) {
				$errors['api_key'] = __( 'API Key Field Cannot be Empty.', 'templately' );
			} else {
				$api_key = sanitize_text_field( $this->post_data['api_key'] );
			}
			$query = \sprintf(
			...
			if( empty( $this->post_data['email'] ) ) {
				$errors['email'] = __( 'Email Field Cannot be Empty.', 'templately' );
			} else {
				$email = sanitize_text_field( $this->post_data['email'] );
			...
			if ( empty( $this->post_data['password'] ) ) {
				...
				$password = sanitize_text_field( $this->post_data['password'] );
			}
			$query = \sprintf(
	...
	public function create_user() {
		...
		if ( empty( $this->post_data['first_name'] ) ) {
			$errors['first_name'] = __( 'First Name Can\'t be Empty.', 'templately' );
		}
		if ( empty( $this->post_data['last_name'] ) ) {
		...
		$first_name = sanitize_text_field( $this->post_data['first_name'] );
		$last_name  = sanitize_text_field( $this->post_data['last_name'] );
	...
	protected function delete_template() {
		...
		// $type = isset( $this->post_data['type'] ) ? trim( $this->post_data['type'] ) : 'template';
		\wp_delete_post( $id );
		$cloud_map     = DB::get_option( '_templately_cloud_map', [] );
		$new_cloud_map = [];
	...
	protected function clouds_sync() {
		...
		if ( ! empty( $api_key ) ) {
			$file_type = isset( $this->post_data['fileType'] ) ? sanitize_text_field( $this->post_data['fileType'] ) : 'elementor';
			$search    = isset( $this->post_data['search'] ) ? sanitize_text_field( $this->post_data['search'] ) : '';
			$per_page  = isset( $this->post_data['per_page'] ) ? intval( $this->post_data['per_page'] ) : 12;
			$pagenum   = isset( $this->post_data['page'] ) ? intval( $this->post_data['page'] ) : 1;
	...
	protected function install_requirements() {
		$requirements = isset( $this->post_data['requirements'] )
			? \json_decode( \wp_unslash( sanitize_text_field( $this->post_data['requirements'] ) ), true )
			: false;
	...
	protected function set_favourite( $unfav = false ) {
		...
		$id   = isset( $this->post_data['id'] ) ? intval( $this->post_data['id'] ) : false;
		$type = isset( $this->post_data['type'] ) ? trim( sanitize_text_field( $this->post_data['type'] ) ) : false;
	...
	protected function get_workspace() {
		...
		$shared    = isset( $this->post_data['shared'] ) ? boolval( $this->post_data['shared'] ) : false;
		$pagenum   = isset( $this->post_data['pagenum'] ) ? intval( $this->post_data['pagenum'] ) : 1;
		$file_type = isset( $this->post_data['file_type'] ) ? sanitize_text_field( $this->post_data['file_type'] ) : 'elementor';
		$per_page  = isset( $this->post_data['per_page'] ) ? intval( $this->post_data['per_page'] ) : 11;
	...
	protected function create_workspace() {
		...
		$title = isset( $this->post_data['title'] ) ? sanitize_text_field( $this->post_data['title'] ) : false;
		if ( $title === false ) {
			Helper::send_error( __( 'You have to give title at least.', 'templately' ) );
		}
		$share_with = isset( $this->post_data['share_with'] ) ? sanitize_text_field( $this->post_data['share_with'] ) : '';
	...
	protected function create_or_edit_workspace( $edit = false ) {
		...
		if ( $edit ) {
			$action   = 'edit';
			$id       = isset( $this->post_data['id'] ) ? intval( $this->post_data['id'] ) : false;
			$platform = ( isset( $this->post_data['platform'] ) && ! empty( $this->post_data['platform'] ) ) ? sanitize_text_field($this->post_data['platform']) : 'elementor';
			if ( $id === false ) {
				Helper::send_error( __( 'Workspace not found.', 'templately' ) );
			}
			$remove = ( isset( $this->post_data['remove'] ) && ! empty( $this->post_data['remove'] ) ) ? sanitize_text_field( $this->post_data['remove'] ) : '';
			$query  = sprintf(
	...
	protected function delete_workspace() {
		...
		$withFiles = isset( $this->post_data['withFiles'] ) ? boolval( intval( $this->post_data['withFiles'] ) ) : false;
	...
	protected function copy_or_move() {
		...
		$action        = isset( $this->post_data['trigger_action'] ) ? sanitize_text_field( $this->post_data['trigger_action'] ) : 'copy';
		$cloud_file_id = isset( $this->post_data['cloud_file_id'] ) ? intval( $this->post_data['cloud_file_id'] ) : false;
		if ( $cloud_file_id === false ) {
			Helper::send_error( __( 'No Cloud Item is selected to' . $action . '.', 'templately' ) );
		}
		$workspace_id = isset( $this->post_data['workspace_id'] ) ? intval( $this->post_data['workspace_id'] ) : false;
		...
		if ( isset( $response['data'], $response['data']['copyOrMoveToWorkspace'], $response['data']['copyOrMoveToWorkspace']['status'] ) ) {
			...
			if( $response['data']['copyOrMoveToWorkspace']['status'] === 'error' ) {
				return $response['data']['copyOrMoveToWorkspace'];
			}
	...
	protected function workspace_details() {
		...
		$file_type      = isset( $this->post_data['file_type'] ) ? sanitize_text_field( $this->post_data['file_type'] ) : 'elementor';
		$search         = isset( $this->post_data['search'] ) ? sanitize_text_field( $this->post_data['search'] ) : '';
		$files_per_page = isset( $this->post_data['per_page'] ) ? intval( $this->post_data['per_page'] ) : 10;
		$files_page     = isset( $this->post_data['page'] ) ? intval( $this->post_data['page'] ) : 1;
	...
	protected function add_files_to_workspace() {
		...
		$files = isset( $this->post_data['files'] ) ? sanitize_text_field( $this->post_data['files'] ) : false;
		if ( $files === false ) {
			Helper::send_error( __( 'Workspace Not Found.', 'templately' ) );
		}
		$query    = sprintf(
			'mutation { addFileToWorkspace( api_key: "%s", workspace_id: %d, files: %s ){ data, status, message } }',
			$api_key,
			$workspace_id,
			\json_encode( $files )
		);
	...
	public function get_templates() {
		return Elementor::templates( false, [
			'page' => ( isset( $this->post_data['page'] ) ? intval( $this->post_data['page'] ) : 1 )
		] );
	}
}
